...
function processElement(targetElement, copiedText, iteration) {
    ...
    provisions.forEach(function (prov) {
        if (prov.index === -1)
            console.log("RegExpMatchArray did not have an index");
    });
...
function isProvisionResultComplete(provisionResult) {
    // e.g. 15.—
    var regex = /\d+\.—\(\d+\)/;
    if (!regex.test(provisionResult[0]))
        return false;
    return false;
}
function sortProvComponents(provisions) {
    var provisionsCopy = lodash_clonedeep_1.default(provisions);
    provisionsCopy.sort(function (a, b) { return a.index - b.index; });
...
function traverseUpToElement(element, nodeName) {

...
function processElement(targetElement) {
    ...
    try {
        for (var provisions_1 = __values(provisions), provisions_1_1 = provisions_1.next(); !provisions_1_1.done; provisions_1_1 = provisions_1.next()) {
            ...
            if (isRoman(prevProv) && isAlpha(text) && text !== "(i)")
                provResult.unshift(text);
            // prevProvResult isAlpha, add the next prov that is a number
            // e.g. (a), next prov should be 15
            if (isAlpha(prevProv) && isBracketedNumber(text))
                provResult.unshift(text);
            if (isBracketedNumber(prevProv) && isNumber(text))
                provResult.unshift(text);
            // prevResult isNumber
...
function getStartIndexOfCopiedText(targetElement, parentFullText) {
    ...
    if (occurences > 2) {
        (parentFullText.split(childFullText)).forEach(function (text) { return console.log(text); });

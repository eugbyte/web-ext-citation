...
function processElement(targetElement) {
    var e_1, _a;
    var parentElement = traverseUpToElement(targetElement, "DIV");
    var parentFullText = parentElement.innerText;
    console.log(parentFullText);
    //E.g. 15.—(1) or (a)
    var regex = /\d+\w?\.—\(\d+\)|\n\(\w+\)/g;
    var matches = findMatches(regex, parentFullText);
    var provisions = matches.map(function (m) { var _a; return new ProvisionComponent((_a = m.index) !== null && _a !== void 0 ? _a : -1, m[0]); });
    console.log(provisions);
    // Need to seperate 15.—(1) into 15. and (1)
    var originalFirstComponent = provisions.shift();
    var _b = __read(splitFirstProvisionComponent(originalFirstComponent), 2), newFirstComponent = _b[0], newSecondComponent = _b[1];
    ...
    var startIndex = getStartIndexOfCopiedText(targetElement, parentFullText);
    ...
    try {
        for (var provisions_1 = __values(provisions), provisions_1_1 = provisions_1.next(); !provisions_1_1.done; provisions_1_1 = provisions_1.next()) {
            ...
            var isFirstProvComponent = isNumber(prevProv);
...
function isBracketedNumber(str) {
    return /\(\d+\)/.test(str);
}
function isNumber(str) {
    return !isNaN(parseInt(str));
}
function sortProvComponentsByDescending(provisions) {
...
// Need to seperate 15.—(1) into 15. and (1)
function splitFirstProvisionComponent(originalFirstComponent) {
...
function getStartIndexOfCopiedText(targetElement, parentFullText) {
...
function traverseUpToElement(element, nodeName, _iteration) {
    if (_iteration === void 0) { _iteration = 1; }
    if (!(element.nodeName === nodeName)) {
        return traverseUpToElement(element.parentElement, nodeName);
    }
    return element;
}
...
document.addEventListener('copy', function (event) {
    ...
    if (!copiedText)
        return;
    traverse_dom_1.processElement(targetElement);

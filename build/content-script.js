...
var regex_service_1 = __webpack_require__(/*! src/services/regex-service */ "./src/services/regex-service.ts");
function getChapter() {
    var regexService = new regex_service_1.RegexService();
    var nav = document.getElementById("tocNav");
    var span = nav.querySelector("span");
    var chapter = span.innerText;
...
var provision_service_1 = __webpack_require__(/*! src/services/provision-service */ "./src/services/provision-service.ts");
var regex_service_1 = __webpack_require__(/*! src/services/regex-service */ "./src/services/regex-service.ts");
function getProvisions(sectionText, parentFullText) {
    var e_1, _a;
    var regexService = new regex_service_1.RegexService();
    var provService = new provision_service_1.ProvisionService();
    // E.g. 15.—(1) or 15. or (a)
    var regex = /\d+\w?\.—\(\d+\)|\d+\.|\n\(\w+\)/g;
    var matches = regexService.findMatches(regex, parentFullText);
    var provisions = matches.map(function (m) { var _a; return new provision_service_1.ProvisionComponent((_a = m.index) !== null && _a !== void 0 ? _a : -1, m[0]); });
    ...
    var originalFirstComponent = provisions.shift();
    var _b = __read(provService.splitFirstProvisionComponent(originalFirstComponent), 2), newFirstComponent = _b[0], newSecondComponent = _b[1];
    provisions.unshift.apply(provisions, [newFirstComponent, newSecondComponent]);
    // In case user copies the more than one provision, including the provision number
    var startIndex = regexService.getStartIndexOfCopiedText(sectionText, parentFullText);
    var endIndex = startIndex + sectionText.length - 1;
    ...
    var provResult = [];
    try {
        for (var provisions_1 = __values(provisions), provisions_1_1 = provisions_1.next(); !provisions_1_1.done; provisions_1_1 = provisions_1.next()) {
            var prov = provisions_1_1.value;
            if (prov.index >= endIndex)
                continue;
            var text = prov.text.replace(/(\r\n|\n|\r)/g, '');
            // The first prov component after the EOS is added
            if (provResult.length === 0) {
                provResult.push(text);
                continue;
            }
            var prevProv = provResult[0];
            // e.g. 15(2)(a)(i)
            ...
            var isFirstProvComponent = regexService.isNumber(prevProv);
        ...
        try {
            if (provisions_1_1 && !provisions_1_1.done && (_a = provisions_1.return)) _a.call(provisions_1);
        }
        finally { if (e_1) throw e_1.error; }
    }
    provResult = provResult.filter(function (prov) { return prov !== "(-1)"; });
...
var dom_service_1 = __webpack_require__(/*! src/services/dom-service */ "./src/services/dom-service.ts");
var regex_service_1 = __webpack_require__(/*! src/services/regex-service */ "./src/services/regex-service.ts");
var getChapter_1 = __webpack_require__(/*! ./getChapter */ "./src/content-script/sso.agc.gov.sg/getChapter.ts");
var getProvisions_1 = __webpack_require__(/*! ./getProvisions */ "./src/content-script/sso.agc.gov.sg/getProvisions.ts");
/*
  The idea here is to
...
function getCitation(targetElement, copiedText) {
    var _a;
    var domService = new dom_service_1.DOMService();
    // Clone the target element so that the actual DOM is not affected
    var cloneTarget = domService.duplicateNodeWithParentRef(targetElement);
    ...
    */
    var rootElement = domService.traverseUpToElement(cloneTarget, "DIV");
    var fullText = rootElement.innerText;
    // The full text of the sub section which the user copied
    var sectionText = cloneTarget.innerText;
    sectionText = removeUnhandledSectionText(cloneTarget);
    if (copiedText.length > sectionText.length) {
        _a = __read([sectionText, copiedText], 2), copiedText = _a[0], sectionText = _a[1];
    }
    console.log("sectionText: \n" + sectionText);
    cloneTarget.parentElement.removeChild(cloneTarget);
    var provision = getProvisions_1.getProvisions(sectionText, fullText);
    var chapter = getChapter_1.getChapter();
...
function removeUnhandledSectionText(targetElement) {
    ...
    var matches = regexService.findMatches(/\([a-z]+\)/g, innerText);
    if (matches.length === 0)
        return innerText;
    var index = matches[0].index;
    var sectionText = targetElement.innerText.slice(0, index);
    return sectionText;
}
...
var DOMService = /** @class */ (function () {
    ...
    DOMService.prototype.duplicateNodeWithParentRef = function (targetElement) {
        var cloneTarget = targetElement.cloneNode(true);
    ...
    DOMService.prototype.traverseUpToElement = function (element, nodeName, iteration) {
...
var ProvisionComponent = /** @class */ (function () {
    function ProvisionComponent(index, text) {
        this.index = index;
        this.text = text;
    }
    ProvisionComponent.prototype.toString = function () {
...
var ProvisionService = /** @class */ (function () {
    ...
    ProvisionService.prototype.sortProvComponentsByDescending = function (provisions) {
        var provisionsCopy = lodash_clonedeep_1.default(provisions);
        provisionsCopy.sort(function (a, b) { return b.index - a.index; });
    ...
    ProvisionService.prototype.splitFirstProvisionComponent = function (originalFirstComponent) {
        var secondProvIndex = originalFirstComponent.index + originalFirstComponent.text.length;
        if (originalFirstComponent.text.includes('.—')) {
            var _a = __read(originalFirstComponent.text.split('.—'), 2), firstProvText = _a[0], secondProvText = _a[1];
            return [
        ...
        if ((/\d+\./).test(originalFirstComponent.text)) {
            var newFirstComponentText = originalFirstComponent.text.replace(".", "");
            return [
...
var RegexService = /** @class */ (function () {
    ...
    RegexService.prototype.findMatches = function (regex, text) {
        var matches = __spreadArray([], __read(text.matchAll(regex)));
        return matches;
    };
    // E.g. (a)
    RegexService.prototype.isBracketedAlpha = function (str) {
        return /\([A-Z]+\)/i.test(str);
    };
    // E.g. (i), (iv)
    RegexService.prototype.isBracketedRoman = function (str) {
        var regex = /^\((M{1,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})|M{0,4}(CM|C?D|D?C{1,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{0,3})(XC|X?L|L?X{1,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|I?V|V?I{1,3}))\)$/i;
        return regex.test(str);
    };
    // E.g. (2)
    RegexService.prototype.isBracketedNumber = function (str) {
        return /\(-?\d+\)/.test(str);
    };
    RegexService.prototype.isNumber = function (str) {
        return /^-?\d+$/.test(str);
    };
    RegexService.prototype.titleCase = function (str) {
        return str.replace(/\w\S*/g, function (t) { return t.charAt(0).toUpperCase() + t.substr(1).toLowerCase(); });
    };
    RegexService.prototype.removeLineBreaks = function (str) {
        return str.replace(/\n+/g, "");
    };
    RegexService.prototype.getStartIndexOfCopiedText = function (childFullText, parentFullText) {
        var occurences = parentFullText.split(childFullText).length;
        if (occurences > 2) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
var sso_agc_gov_sg_1 = __webpack_require__(/*! ./sso.agc.gov.sg */ "./src/content-script/sso.agc.gov.sg/index.ts");
document.addEventListener('copy', function (event) {
    ...
    if (!copiedText)
        return;
    var provision = sso_agc_gov_sg_1.getCitation(targetElement, copiedText);
    console.log("result: \n" + provision);

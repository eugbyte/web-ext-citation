...
/***/ "./src/content-script/sso.agc.gov.sg/getChapter.ts":
/*!*********************************************************!*\
  !*** ./src/content-script/sso.agc.gov.sg/getChapter.ts ***!
  \*********************************************************/
...
const regex_service_1 = __webpack_require__(/*! src/services/regex-service */ "./src/services/regex-service.ts");
function getChapter() {
    const regexService = new regex_service_1.RegexService();
...
/***/ "./src/content-script/sso.agc.gov.sg/getProvisions.ts":
/*!************************************************************!*\
  !*** ./src/content-script/sso.agc.gov.sg/getProvisions.ts ***!
  \************************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getProvisions = void 0;
const provision_service_1 = __webpack_require__(/*! src/services/provision-service */ "./src/services/provision-service.ts");
const regex_service_1 = __webpack_require__(/*! src/services/regex-service */ "./src/services/regex-service.ts");
function getProvisions(sectionText, parentFullText) {
    const regexService = new regex_service_1.RegexService();
    const provService = new provision_service_1.ProvisionService();
    // E.g. 15.—(1) or 15. or (a)
    const regex = /\d+\w?\.—\(\d+\)|\d+\.|\n\(\w+\)/g;
    const matches = regexService.findMatches(regex, parentFullText);
    let provisions = matches.map(m => { var _a; return new provision_service_1.ProvisionComponent((_a = m.index) !== null && _a !== void 0 ? _a : -1, m[0]); });
    ...
    const originalFirstComponent = provisions.shift();
    ...
    const startIndex = regexService.getStartIndexOfCopiedText(sectionText, parentFullText);
    const endIndex = startIndex + sectionText.length - 1;
    console.log(startIndex, endIndex);
    provisions.push(new provision_service_1.ProvisionComponent(endIndex, '<EOS>')); // end of sentence token
    ...
    for (const prov of provisions) {
        ...
        if (regexService.isBracketedRoman(prevProv) && regexService.isBracketedAlpha(text) && text !== '(i)')
            provResult.unshift(text);
        // prevProvResult isAlpha, add the next prov that is a number
        // e.g. (a), next prov should be (2)
        if (regexService.isBracketedAlpha(prevProv) && regexService.isBracketedNumber(text))
            provResult.unshift(text);
        // e.g. (2), next prov should be 15
        if (regexService.isBracketedNumber(prevProv) && regexService.isNumber(text))
            provResult.unshift(text);
        // prevResult isNumber
        // Only the first component of the provision is a number, e.g. 15 (a)(i)
        const isFirstProvComponent = regexService.isNumber(prevProv);
...
/***/ "./src/content-script/sso.agc.gov.sg/index.ts":
/*!****************************************************!*\
  !*** ./src/content-script/sso.agc.gov.sg/index.ts ***!
  \****************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getCitation = void 0;
const dom_service_1 = __webpack_require__(/*! src/services/dom-service */ "./src/services/dom-service.ts");
const regex_service_1 = __webpack_require__(/*! src/services/regex-service */ "./src/services/regex-service.ts");
const getChapter_1 = __webpack_require__(/*! ./getChapter */ "./src/content-script/sso.agc.gov.sg/getChapter.ts");
const getProvisions_1 = __webpack_require__(/*! ./getProvisions */ "./src/content-script/sso.agc.gov.sg/getProvisions.ts");
/*
  The idea here is to
...
*/
function getCitation(targetElement, copiedText) {
    const domService = new dom_service_1.DOMService();
    // Clone the target element so that the actual DOM is not affected
    const cloneTarget = domService.duplicateNodeWithParentRef(targetElement);
    ...
    */
    const rootElement = domService.traverseUpToElement(cloneTarget, "DIV");
    let fullText = rootElement.innerText;
    // The full text of the sub section which the user copied
    let sectionText = cloneTarget.innerText;
    sectionText = removeUnhandledSectionText(cloneTarget);
    // Remove the clone to prevent memory waste
    cloneTarget.parentElement.removeChild(cloneTarget);
    ...
    const provision = getProvisions_1.getProvisions(sectionText, fullText);
    const chapter = getChapter_1.getChapter();
...
function removeUnhandledSectionText(targetElement) {
    const regexService = new regex_service_1.RegexService();
...
class DOMService {
    duplicateNodeWithParentRef(targetElement) {
    ...
    traverseUpToElement(element, nodeName, iteration = 1) {
        if (!(element.nodeName === nodeName)) {
            return this.traverseUpToElement(element.parentElement, nodeName, iteration + 1);
        }
        return element;
    }
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.ProvisionService = exports.ProvisionComponent = void 0;
...
exports.ProvisionComponent = ProvisionComponent;
class ProvisionService {
    sortProvComponentsByDescending(provisions) {
        const provisionsCopy = lodash_clonedeep_1.default(provisions);
    ...
    splitFirstProvisionComponent(originalFirstComponent) {
...
class RegexService {
    ...
    // E.g. (a)
    isBracketedAlpha(str) {
        return /\([A-Z]+\)/i.test(str);
    }
    // E.g. (i), (iv)
    isBracketedRoman(str) {
    ...
    reduceLineBreaks(str) {
        return str.replace(/[\n\r]+/g, "\n");
    }
    // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions/Character_Classes
    reduceWhiteSpacesExceptLineBreaks(str) {
        return str.replace(/[ \f\t\v\u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\ufeff]/g, ' ');
    }
    getStartIndexOfCopiedText(childFullText, parentFullText) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const sso_agc_gov_sg_1 = __webpack_require__(/*! ./sso.agc.gov.sg */ "./src/content-script/sso.agc.gov.sg/index.ts");
document.addEventListener('copy', (event) => {
    var _a;
    const copiedText = (_a = document.getSelection()) === null || _a === void 0 ? void 0 : _a.toString();
    const targetElement = event.target;
    if (!copiedText)
        return;
    const provision = sso_agc_gov_sg_1.getCitation(targetElement, copiedText);
    console.log(`result: \n${provision}`);
